...
DOS作为主流桌面操作系统的时代已经在2000年就结束了，但DOS并没有彻底消亡。凭借着极低的硬件开销、无虚拟化延迟的直接硬件访问能力，以及庞大的历史遗产，仍活跃在四大特定领域
...
安装qemu
```bash
sudo apt update && sudo apt install qemu-system-x86 qemu-img qemu-desktop
```
...
DOS软件的发展跨度非常大，主要因为DOS的版本，CPU + 内存 + 硬件环境导致的\
可以粗略分为几个时期

| 时期 | 典型CPU | DOS | 软件特点 |
| - | - | - | - |
| 早期 | 8086/8088 | DOS 1.x-2.x | `.COM`、BIOS/DOS调用，非常原始 |
| 中期 | 286/386 | DOS 3.x-5.x | 内存管理、硬盘、鼠标、VGA等逐渐成熟 |
| 黄金时期 | 386/486/Pentium | DOS 5.x-6.x | 大量商业游戏，Sound Blaster、VGA、XMS/EMS |
| 晚期 | 486/Pentium | DOS 6.x / FreeDOS | DOS游戏兼容环境、各种DOS扩展 |
