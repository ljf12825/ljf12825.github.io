...
Mark可以让你在文件或跨文件中快速跳转到特定位置，你在文件某个位置做个标记，之后可以快速跳回来。Vim会记住标记所在的行号和列号
...
```vim
:marks  " 显示所有标记列表
:marks aBc  " 显示特定标记 a, B, c的信息
```

#### 快速标记跳转

在正常模式下，使用`g'{marks}`和`g`{marks}`可以跳转但不改变跳转列表
...
`formatoptions`是Vim里的排版器，它通常在以下集中操作时触发

- 输入超过`textwidth`长度时（自动换行）
- 按`gq`命令重新格式化段落时
- 按`o`或`O`新开一行是（自动继承上一行的缩进和注释前缀）

当我在上一行写注释`// 注释`后，按下回车键换行，下一行自动在相同位置补充了`//`，这是Vim的自动注释延续功能，由`formatoptions`选项中的`r`和`o`标志控制
