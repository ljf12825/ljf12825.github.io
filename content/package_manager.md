...
Arch的官方仓库是由Arch团队官方维护、经过审查与编译的预编译二进制包仓库\
它们定义在`/etc/pacman.conf`下，用`pacman`直接安装\
主要分为以下几个分支：

- `core`：系统的核心组件，包含启动系统、联网、文件系统工具以及核心系统软件（如`linux`内核、`glibc`、`pacman`等），是系统能启动的最小集合
- `extra`：官方仓库中最大的一个，包含桌面环境、浏览器、媒体播放器、联网、文件系统工具等绝大多数日常使用的常用软件。以前还有`community`仓库，2023年已经合并进入`extra`
- `multilib`：64位系统上的32位兼容库仓库（如Steam, Wine的部分依赖），默认不启用，需要在`/etc/pacman.conf`里手动取消注释
- `*-testing`：软件测试仓库，包在正式进入上面三个仓库前先在这里测试，一般不启用，除非你愿意帮忙测试
