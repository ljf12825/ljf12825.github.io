...
- 归并排序每次将问题分成两个子问题，每个子问题的大小为\(\frac{n}{2}\)，并且每一层合并的操作需要`O(n)`时间
- 递归关系式 \(T(n)=2T(\frac{n}{2})+O(n)\)
...
- 快速排序每次选择一个基准元素，将数组分成两部分，然后递归排序每个部分。假设每次都能将数组均匀分割，则递归关系式为 \(T(n)=2T(\frac{n}{2})+O(n)\)
- 其中，`O(n)`是每次划分过程的工作量，\(2T(\frac{n}{2})\)表示两个子问题的递归调用
...
- 总的工作量是各层的工作量之和\(O(n)+O(n)+O(n)+···+O(n)=O(nlogn)\)
